...
export function setupUpload() {
    ...
    function updateDisabledOptions() {
        ...
        if (aggColSelect.value === gVal && gVal) {
            const first = Array.from(aggColSelect.options).find(opt => !opt.disabled);
            if (first) aggColSelect.value = first.value;
        }
        if (groupBySelect.value === aVal && aVal) {
            const first = Array.from(groupBySelect.options).find(opt => !opt.disabled);
            if (first) groupBySelect.value = first.value;
        }
    }
